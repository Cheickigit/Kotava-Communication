...
const imagePreview = ref(props.actualite?.image_url || props.actualite?.image || null);

const form = useForm({
  _method: 'put',
  title: props.actualite?.title || props.actualite?.titre || '',
  slug: props.actualite?.slug || '',
  extrait: props.actualite?.extrait || '',
  content: props.actualite?.content || props.actualite?.description || '',
  category: props.actualite?.category || 'actualite',
  statut: props.actualite?.statut || props.actualite?.status || 'brouillon',
  image: null,
  remove_image: false,
  date_publication: props.actualite?.date_publication || '',
  featured: Boolean(props.actualite?.featured),
  meta_title: props.actualite?.meta_title || '',
  meta_description: props.actualite?.meta_description || '',
});
...
const submit = () => {
  form.post(`/dashboard/actualites/${props.actualite.id}`, {
    forceFormData: true,
    preserveScroll: true,
  });
};
...
<template>
  <Head :title="`Modifier - ${form.title || 'Actualité'}`" />
      ...
      <section class="relative overflow-hidden rounded-[2rem] bg-[#07101d] p-6 text-white shadow-2xl sm:p-8">
        ...
        <div class="relative z-10">
          ...
          <p class="mt-4 max-w-2xl text-base leading-7 text-white/60">
            Mettez à jour le contenu, le statut, l’image et les paramètres SEO de cette publication.
          </p>
        </div>
      </section>
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-sm text-slate-500">
                  Titre, extrait et contenu complet.
                </p>
              </div>
            </div>
        ...
        <aside class="space-y-6">
          ...
          <div class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <button
              v-if="imagePreview"
              type="button"
              class="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-red-100 bg-red-50 px-4 py-3 text-sm font-black text-red-600 transition hover:bg-red-600 hover:text-white"
              @click="removeImage"
            >
              <Trash2 :size="17" />
              Supprimer l’image
